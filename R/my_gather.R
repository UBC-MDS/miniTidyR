...
my_gather <- function(data, keyname="key",valuename="value",chosen_cols) {
  ...
  for (col_names in chosen_cols) { if(is.element(col_names,col_names_list)==FALSE) {warning("A wrong column name was specified, returning input data", immediate. =TRUE) 
    return(data) }
